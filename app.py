...
def calculate_invoice(mode, amount, tds_label, description, client_name):
    ...
    if mode == "USD (Export)":
        ...
        inv = dict(
            ...
            net_receivable=inv_total - amount * tds_rate,
            exchange_rate=None,
        )
    inv.update(
...
def build_excel(invoices: list, owner: str) -> bytes:
    ...
    cs(
        ...
        2,
        1,
        f"For: {owner}   |   {datetime.now().strftime('%d %b %Y %H:%M')}   |   Rate: ₹{USD_TO_INR}/USD",
        fg=M,
        bg="080C10",
        al="center",
        sz=9,
    ...
    for idx, inv in enumerate(invoices, 1):
        ...
        tot["usd"] += inv.get("usd_amount") or 0
        tot["inr"] += inv["inr_amount"]
        tot["cgst"] += inv["cgst"]
        tot["sgst"] += inv["sgst"]
        tot["gst"] += inv["total_gst"]
        tot["ivt"] += ivt
        tot["tds"] += inv["tds_amount"]
        tot["net"] += inv["net_receivable"]
    ...
    for inv in invoices:
        ...
        if cn not in clients:
            clients[cn] = {"rev": 0, "gst": 0, "tds": 0, "net": 0, "n": 0}
        clients[cn]["rev"] += inv["inr_amount"]
        clients[cn]["gst"] += inv["total_gst"]
        clients[cn]["tds"] += inv["tds_amount"]
        clients[cn]["net"] += inv["net_receivable"]
        clients[cn]["n"] += 1
    ...
    sd = [
        ...
        ("Total Revenue (INR)", tot["inr"], "Taxable"),
        ("Total GST", tot["gst"], "CGST+SGST"),
        ("  ↳ CGST", tot["cgst"], "Central"),
        ("  ↳ SGST", tot["sgst"], "State"),
        ("TDS Receivable", tot["tds"], "Claim ITR"),
...
def login_block():
    st.markdown(
        "<div style='text-align:center;padding:3rem 0 1.5rem'><div class='zenith-logo'>⚡ ZENITH IN</div><div class='zenith-sub'>DataSnap Pro · Financial OS v2.0</div></div>",
        unsafe_allow_html=True,
    )
    ...
    with col:
        ...
        st.markdown(
            "<div style='font-size:.72rem;color:#8b98a5;margin-top:1rem'>Demo → admin / zenith@2026<br>Demo → client1 / client1pass</div>",
            unsafe_allow_html=True,
        )
...
def render_sidebar():
    with st.sidebar:
        st.markdown(
            "<div style='padding:1rem 0 .5rem'><div class='zenith-logo'>⚡ ZENITH IN</div><div class='zenith-sub'>DataSnap Pro v2.0</div></div>",
            unsafe_allow_html=True,
        )
        st.divider()
        ...
        st.markdown(
            f"<div style='margin-bottom:1rem'><div style='font-size:.8rem;color:#8b98a5'>Logged in as</div><div style='font-size:1rem;font-weight:700'>{name}</div><span class='{'badge-admin' if role=='Admin' else 'badge-inr'}'>{role}</span></div>",
            unsafe_allow_html=True,
        )
        pages = [
            "📊 Dashboard",
        ...
        st.markdown(
            f"<div style='font-size:.72rem;color:#8b98a5'>💱 ₹{USD_TO_INR}/USD<br>📅 {date.today().strftime('%d %b %Y')}<br>🧾 <b style='color:#00a3ff'>{n}</b> invoice(s) saved<br>🔒 Session active</div>",
            unsafe_allow_html=True,
        )
...
def page_dashboard():
    ...
    for col, lbl, val, sub in [
        ...
        with col:
            st.markdown(
                f"<div class='metric-card'><div class='metric-label'>{lbl}</div><div class='metric-value'>{val}</div><div class='metric-sub'>{sub}</div></div>",
                unsafe_allow_html=True,
            )
    st.divider()
    ca, cb = st.columns(2)
    ...
    with cb:
        ...
        if ex + dm > 0:
            ...
            st.info("Will populate after invoices.")
    ...
    if inv:
        ...
        for i in inv:
            ...
            if cn not in cl:
                cl[cn] = {"Revenue": 0, "GST": 0, "TDS": 0, "Net": 0, "Count": 0}
            cl[cn]["Revenue"] += i["inr_amount"]
            cl[cn]["GST"] += i["total_gst"]
            cl[cn]["TDS"] += i["tds_amount"]
            cl[cn]["Net"] += i["net_receivable"]
            cl[cn]["Count"] += 1
        st.dataframe(
            pd.DataFrame(
                [
    ...
    with c1:
        ...
        if st.button("⚡  Calculate & Save Invoice", use_container_width=True):
            if not client_name.strip():
                ...
                save_invoice(i)
                st.success(
                    f"✅ Saved! {len(st.session_state['invoices'])} total invoices in session."
                )
    with c2:
        st.markdown(
        ...
        if i:
            ...
            rows = (
                [
                    ...
                    ("USD Amount", fmt_usd(i["usd_amount"])),
                    ("Exchange Rate", f"₹{USD_TO_INR}"),
                    ("INR Equivalent", fmt_inr(i["inr_amount"])),
                    ("GST", i["gst_type"]),
                    ("TDS", fmt_inr(i["tds_amount"])),
                    ("✅ Net", fmt_inr(i["net_receivable"])),
                ]
                if is_usd
                else [
                    ("Client", i.get("client_name", "—")),
                    ("Taxable", fmt_inr(i["inr_amount"])),
                    ("CGST 9%", fmt_inr(i["cgst"])),
                    ("SGST 9%", fmt_inr(i["sgst"])),
                    ("Total GST", fmt_inr(i["total_gst"])),
                    ("Invoice Total", fmt_inr(i.get("invoice_total", 0))),
                    ("TDS", fmt_inr(i["tds_amount"])),
                    ("✅ Net", fmt_inr(i["net_receivable"])),
                ]
            )
            ...
            st.info("Calculate an invoice to see preview.")
...
def page_ai():
    ...
    st.markdown(
        "<div style='color:#8b98a5;font-size:.82rem'>Gemini 1.5 Flash · Multi-modal PDF/Image</div>",
        unsafe_allow_html=True,
    )
    model = get_gemini()
    if not model:
    ...
    with tab1:
        q = st.text_area(
            "Ask about GST, TDS, ITR...",
            height=100,
            placeholder="e.g. Section 44ADA kya hai?",
        )
        inv = st.session_state["invoices"]
        ctx = (
            f"\n\nUser has {len(inv)} invoices. Revenue ₹{sum(i['inr_amount'] for i in inv):,.0f}, GST ₹{sum(i['total_gst'] for i in inv):,.0f}."
            if inv
            else ""
        )
        if st.button("Ask Gemini ⚡"):
            if q.strip():
                with st.spinner("Consulting AI..."):
                    p = (
                        "You are DataSnap Pro AI advisor for Indian freelancers. "
                        "Give accurate answers on GST/TDS/ITR. Cite sections."
                        + ctx
                        + "\n\nQ: "
                        + q
                    )
                    ...
                    st.markdown(
                        f"<div class='ai-response'>{r.text}</div>",
                        unsafe_allow_html=True,
                    )

    with tab2:
        st.markdown(
            "<div class='section-header'>Upload Invoice → AI Extracts → Add to Dashboard</div>",
            unsafe_allow_html=True,
        )
...
For INR domestic: set usd_amount to null. Fill all fields accurately."""
                    ...
                    st.markdown(
                        f"<div class='ai-response'>{raw}</div>", unsafe_allow_html=True
                    )
                    ...
                    try:
                        ...
                        parsed.setdefault(
                            ...
                            (
                                "INR – Domestic"
                                if not parsed.get("usd_amount")
                                else "USD – Export of Service"
                            ),
                        )
                        parsed.setdefault(
                            "gst_type",
                            (
                                "LUT / Export – 0% GST"
                                if parsed.get("usd_amount")
                                else "18% GST (CGST 9% + SGST 9%)"
                            ),
                        )
                        parsed.setdefault("tds_rate", 0.0)
                        parsed.setdefault(
                            "exchange_rate",
                            USD_TO_INR if parsed.get("usd_amount") else None,
                        )
                        parsed.setdefault(
                            "invoice_total",
                            parsed.get("inr_amount", 0) + parsed.get("total_gst", 0),
                        )
                        parsed.setdefault(
                        ...
                        st.warning(
                            f"Auto-parse failed ({e}). Add manually via New Invoice."
                        )
        ...
        if ex:
            ...
            st.dataframe(
                pd.DataFrame(preview, columns=["Field", "Value"]),
                use_container_width=True,
                hide_index=True,
            )
            ...
            with col_a:
                if st.button("📥 Add to Dashboard", use_container_width=True):
                    ...
                    st.session_state["ai_extracted"] = None
                    st.success(
                        f"✅ Added! Total: {len(st.session_state['invoices'])} invoices"
                    )
                    st.rerun()
            with col_b:
...
def page_export():
    ...
    with tab1:
        ...
        if not inv:
            ...
            st.caption(
                "Includes: CA Audit · Client Summary · Overall Summary — 3 sheets"
            )
    ...
    with tab2:
        ...
        st.markdown(
            "<div style='background:#0a1833;border:1px solid #1e2d3d;border-radius:8px;padding:.9rem 1.1rem;margin-bottom:1rem;font-size:.78rem;color:#8b98a5'>Pre-wired for <b style='color:#00a3ff'>Twilio/WATI</b>. Uncomment code in <code>send_whatsapp_placeholder()</code> and add Secrets.</div>",
            unsafe_allow_html=True,
        )
    ...
    st.json(
        {
            ...
            "GSheets": "stub (uncomment to activate)",
